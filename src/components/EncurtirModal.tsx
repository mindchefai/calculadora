...
const categoryIcons: Record<string, JSX.Element> = {
  'Encurtidos de Verduras': <Carrot className="w-5 h-5 text-green-600" />,
  'Encurtidos Genéricos': <Leaf className="w-5 h-5 text-emerald-600" />,
  'Otros Encurtidos': <Sparkles className="w-5 h-5 text-purple-600" />
};
...
const EncurtirModal: React.FC<EncurtirModalProps> = ({ isOpen, onClose }) => {
  ...
  const calculateIngredients = () => {
    ...
    const calculatedIngredients = recipe.ingredients.map(ing => {
      ...
      if (isNaN(amountNum)) {
        // Si no es un número simple (ej: "1/2"), mantener original
        return {
          name: ing.name,
          amount: ing.amount,
          unit: ing.unit
                        ...
                        <div className="space-y-4">
                          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-lg p-5">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <span className="font-semibold text-green-900">Ingredientes calculados</span>
                            </div>
